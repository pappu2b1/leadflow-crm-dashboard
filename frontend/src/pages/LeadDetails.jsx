    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <section className="card p-5">
          ...
          {isDemo ? <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm font-medium text-amber-900">Notes are view-only in demo mode.</p> : <form onSubmit={addNote} className="mt-4 space-y-3"><textarea className="input" rows={4} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Add a call note, proposal update, or follow-up outcome." /><button className="btn-primary" disabled={savingNote}><Plus size={17} /> {savingNote ? "Adding..." : "Add Note"}</button></form>}
      ...
      <ConfirmModal open={!isDemo && deleteOpen} title="Delete lead?" description={`This will permanently delete ${lead.fullName}.`} onCancel={() => setDeleteOpen(false)} onConfirm={deleteLead} confirmText="Delete Lead" />
    </div>
  );
};
