...
import { generateDemoToken, generateToken } from "../utils/generateToken.js";
...
export const loginAdmin = asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, token: generateToken(admin._id), user: publicAdmin(admin) });
});

export const loginDemo = asyncHandler(async (req, res) => res.json({
  success: true,
  token: generateDemoToken(),
  user: { id: "demo", name: "Demo User", role: "demo", mode: "readonly" }
}));

export const getMe = asyncHandler(async (req, res) => res.json({
  success: true,
  user: req.user?.role === "demo" ? req.user : publicAdmin(req.admin)
}));
...
export const updateProfile = asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, message: "Profile updated successfully", user: publicAdmin(req.admin) });
});
